...
angular.module('apiInfo.services', []).factory('apiInfoService', ($http, $q) => {
  let pageList = []
    , actionList = []
    , actionId = ''
    , _curActionId = '2222'
  ...
  interfaceListService.savePages = (actionId) => {
    ...
    for (let i = 0; i < plg; i++) {
      ...
      for (let k = 0; k < actionListCount; k++) {
        ...
        if (action.id === actionId) {
          ...
          let ap = $http.post('/api', action).success((data) => {
            if (data.status === 'success') {
              deferred.resolve(data);
            } else {
              deferred.reject(data.error);
    ...
    $q.all(plist).then(function (results) {
  ...
  interfaceListService.create = (api) => {
    const deferred = $q.defer();
    const promise = deferred.promise;
    $http.post('/api', api).success((data) => {
      if (data.status === 'success') {
  ...
  interfaceListService.addNewParam = (type, parentParamId, paramType) => {
    var newParamId = -1;
    if (type == "child") {
      newParamId = interfaceListService.addChildParameter(actionId, parentParamId, paramType);
    } else if (type == "reqParam") {
